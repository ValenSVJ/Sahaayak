...
# Saved Payment Info route
    ...
    # Get products with wholesaler information
    cursor.execute('''
    ...
    # If no products found, show all products (for demo purposes)
    if not products:
        cursor.execute('''
            SELECT p.*, w.name as wholesaler_name, w.location, w.trust_score 
            FROM products p 
        ...
        ''')
        products = cursor.fetchall()
        wholesaler_category = f"All Products (No {wholesaler_category} available)"
